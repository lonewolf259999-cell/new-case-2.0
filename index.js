...
// Spreadsheet ID ของไฟล์ Config (จากรูปที่คุณส่งมา)
...
async function loadConfigFromSheets() {
    try {
        ...
        // กรณีดึงไม่ได้ ให้ดึงจากไฟล์สำรองถ้ามี
...
async function startApp() {
    await loadConfigFromSheets(); // โหลดค่าก่อนเริ่มบอท
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'settings_modal') {
        ...
        // บันทึกลง Google Sheets ทันทีเพื่อให้ข้อมูลถาวร
...
// --- ส่วนที่เพิ่มเพื่อแก้ปัญหา No open ports detected ---
const http = require('http');
...
// ใช้พอร์ตที่ Render กำหนดให้ หรือ 3000 ถ้าทดสอบในเครื่อง
const PORT = process.env.PORT || 3000;

server.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ Dummy server is listening on port ${PORT} to keep Render happy.`);
});
// -----------------------------------------------------
